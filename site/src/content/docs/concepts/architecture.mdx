...
State is handled by `MediaProvider`, which creates a central store that all components can access. When you wrap your player in a `MediaProvider`, components automatically connect to the state, no manual wiring required.
</FrameworkCase>
<FrameworkCase frameworks={["html"]}>
State is handled by `media-provider`, which creates a central store that all components can access. When you wrap your player in a `media-provider`, components automatically connect to the state, no manual wiring required.
</FrameworkCase>
...
Most should use a skin as a starting point. However, if you want more control...

#### Components
Individual, unstyled UI primitives that can be used standalone or composed into custom designs:
