...
import type { CSSModulesOutput, TailwindCompilationConfig } from './types.js';
import type { AtRule, ChildNode, Container, Declaration, Node, Root, Rule } from 'postcss';
...
function decodeCssIdentifier(value: string): string {
  return value.replace(/\\([0-9A-F]{1,6}\s?|.)/gi, (_, escapeSequence: string) => {
...
function extractVarArguments(node: FunctionNode): { name: string; fallback?: string } {
  ...
  return trimmedFallback ? { name: trimmedName, fallback: trimmedFallback } : { name: trimmedName };
}
 ...
 */
export async function compileTailwindToCSS(config: TailwindCompilationConfig): Promise<CSSModulesOutput> {
  ...
  const rawContent = buildRawContent(
    Array.from(enhancedByKey.entries()).map(([key, enhanced]) => [key, enhanced.simpleClasses.join(' ')])
  );
...
@tailwind utilities;`;
  ...
  const shouldResolve: ('spacing' | 'colors' | 'all')[] =
    config.resolveCSSVariables !== undefined ? config.resolveCSSVariables : ['all']; // Default: resolve everything
  ...
  if (shouldResolve.length > 0) {
    ...
    const cssWithTheme = `:root { ${Object.entries(embeddedTheme)
      .map(([k, v]) => `${k}: ${v};`)
      .join(' ')} }\n${formattedCss}`;
