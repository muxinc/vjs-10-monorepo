...
export interface ArbitraryUtilityValue {
  kind: 'arbitrary';
...
export interface NamedUtilityValue {
  kind: 'named';
...
interface ArbitraryVariantValue {
  kind: 'arbitrary';
  value: string;
}

interface NamedVariantValue {
  kind: 'named';
  value: string;
}
...
export function* parseCandidate(input: string, designSystem: DesignSystem): Iterable<Candidate> {
  ...
  const parsedCandidateVariants: Variant[] = [];

  for (let i = rawVariants.length - 1; i >= 0; --i) {
    const rawVariant = rawVariants[i];
    if (!rawVariant) continue;
    const parsedVariant = parseVariant(rawVariant, designSystem);
  ...
  const parsedModifier = modifierSegment === null ? null : parseModifier(modifierSegment);
  ...
  if (baseWithoutModifier[0] === '[') {
    ...
    const charCode = baseWithoutModifier.charCodeAt(1);
    ...
    const property = baseWithoutModifier.slice(0, idx);
    const value = decodeArbitraryValue(baseWithoutModifier.slice(idx + 1));
  ...
  if (baseWithoutModifier[baseWithoutModifier.length - 1] === ']') {
    ...
    const value = baseWithoutModifier.slice(idx + 1);
  ...
  for (const [root, value] of roots) {
    ...
    {
      const startArbitraryIdx = value.indexOf('[');
      const valueIsArbitrary = startArbitraryIdx !== -1;
      ...
      if (valueIsArbitrary) {
        ...
        for (let i = 0; i < arbitraryValue.length; i++) {
          const code = arbitraryValue.charCodeAt(i);
        ...
        const fraction =
          modifierSegment === null || candidate.modifier?.kind === 'arbitrary' ? null : `${value}/${modifierSegment}`;
...
function parseModifier(modifier: string): CandidateModifier | null {
  if (modifier[0] === '[' && modifier[modifier.length - 1] === ']') {
    const arbitraryValue = decodeArbitraryValue(modifier.slice(1, -1));
  ...
  if (modifier[0] === '(' && modifier[modifier.length - 1] === ')') {
    ...
    const arbitraryValue = decodeArbitraryValue(modifier);
...
export function parseVariant(variant: string, designSystem: DesignSystem): Variant | null {
  ...
  // Static, functional and compound variants
  {
    ...
    //             ^^^^^^^^^^ -> Modifier
    const [variantWithoutModifier, modifier = null, additionalModifier] = segment(variant, '/');
    ...
    const roots = findRoots(variantWithoutModifier, (root) => {
    ...
    for (const [root, value] of roots) {
      switch (designSystem.variants.kind(root)) {
        ...
        case 'functional': {
          const parsedModifier = modifier === null ? null : parseModifier(modifier);
          ...
          if (value[value.length - 1] === ']') {
            ...
            const arbitraryValue = decodeArbitraryValue(value.slice(1, -1));
        ...
        case 'compound': {
          ...
          const subVariant = parseVariant(value, designSystem);
...
function* findRoots(input: string, exists: (input: string) => boolean): Iterable<Root> {
  ...
  while (idx > 0) {
    ...
    if (exists(maybeRoot)) {
      const root: Root = [maybeRoot, input.slice(idx + 1)];
