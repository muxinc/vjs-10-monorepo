...
import type { SelectorCategory, StyleKeyUsage } from '../../types.js';
 ...
 */
export function categorizeStyleKey(styleKey: StyleKeyUsage, componentNames: string[]): SelectorCategory {
 ...
 */
function isComponentSelectorIdentifier(_key: string, usedOn: string[], componentNames: string[]): boolean {
 ...
 */
function isComponentTypeSelector(key: string, usedOn: string[], _componentNames: string[]): boolean {
...
function normalizeCompoundName(componentName: string): string {
  ...
  // TimeRange → Range
  const simplifiedNamespace = namespace.replace(/^Media/, '').replace(/^Time/, '');
