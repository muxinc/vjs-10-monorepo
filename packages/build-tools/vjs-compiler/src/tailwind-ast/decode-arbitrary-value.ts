import * as ValueParser from './value-parser.js'
      ...
      case 'function': {
        ...
        if (
          ...
          for (let i = 0; i < node.nodes.length; i++) {
            ...
            if (i == 0 && currentNode.kind === 'word') {
              currentNode.value = convertUnderscoresToWhitespace(currentNode.value, true)
              continue
            }
            recursivelyDecodeArbitraryValues([currentNode])
          }
          break
        }
...
function addWhitespaceAroundMathOperators(input: string): string {
  // Simple implementation - add spaces around +, -, *, / when not already present
  return input
    .replace(/([+\-*/])/g, ' $1 ')
    .replace(/\s+/g, ' ')
    .trim()
