...
describe('react Inline CSS Pipeline - E2E Validation', () => {
  const outputPath = join(process.cwd(), '../../../temp-skin-permutations/output/react-inline/MediaSkinDefault.tsx');
  ...
  it('validates generated React inline CSS output has valid TypeScript syntax', async () => {
    ...
    if (validation.eslint.warnings.length > 0) {
      console.log(`ℹ️  ${validation.eslint.warnings.length} ESLint warnings (allowed):`);
      validation.eslint.warnings.forEach((w) => console.log(`  - ${w.message} (${w.ruleId})`));
    }
  });
  ...
  it('ensures styles object has properly quoted keys', () => {
    ...
    const stylesMatch = content.match(/const styles: Record<string, string> = \{([^}]+)\}/);
    ...
    if (stylesMatch) {
      ...
      expect(invalidKeys, `Should not have unquoted hyphenated keys. Found: ${invalidKeys?.join(', ')}`).toBeNull();
    }
  });
  ...
  it('validates styles object maps all CSS class names', () => {
    ...
    const cssMatch = content.match(/const inlineStyles = `([^`]+)`/);
    ...
    if (cssMatch) {
      ...
      if (stylesMatch) {
        ...
        const stylesKeyRegex = /['"]?([a-z_][\w-]*)['"]?\s*:/gi;
  ...
  it('validates no runtime Tailwind dependencies', () => {
    ...
    if (cssMatch) {
      ...
      expect(unresolvedCalcVars, 'Should not have unresolved calc(var(...)) expressions').toBeNull();
    }
  });
