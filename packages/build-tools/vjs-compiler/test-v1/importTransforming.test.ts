import type { ImportMappingConfig } from '../src/importTransforming';
...
describe('importTransforming', () => {
  ...
  describe('defaultTransformRelativeImport', () => {
    it('should transform component imports to kebab-case with media- prefix', () => {
      expect(defaultTransformRelativeImport('../../components/PlayButton')).toBe('../components/media-play-button');
    });

    it('should handle already kebab-cased imports with media- prefix', () => {
      expect(defaultTransformRelativeImport('../components/media-play-button')).toBe('../components/media-play-button');
    });

    it('should return unchanged for non-component paths', () => {
      expect(defaultTransformRelativeImport('./utils/helper')).toBe('./utils/helper');
    });
    ...
    it('should special-case MediaContainer to root directory', () => {
      expect(defaultTransformRelativeImport('../../components/MediaContainer')).toBe('../media-container');
    });

    it('should add media- prefix to components without it', () => {
      expect(defaultTransformRelativeImport('../../components/PlayButton')).toBe('../components/media-play-button');
      expect(defaultTransformRelativeImport('../../components/TimeRange')).toBe('../components/media-time-range');
    });

    it('should not double-add media- prefix', () => {
      expect(defaultTransformRelativeImport('../../components/MediaPlayButton')).toBe(
        '../components/media-play-button'
      );
    });
  });
  ...
  describe('transformImports with default functions', () => {
    ...
    it('should use default transformRelativeImport when not provided', () => {
      const imports: ImportInfo[] = [{ source: '../../components/PlayButton', specifiers: [] }];
  ...
  describe('transformImports with custom functions', () => {
    ...
    it('should use custom transformRelativeImport function', () => {
      const imports: ImportInfo[] = [{ source: './MyComponent', specifiers: [] }];
